  ...
  <head>
    ...
    <script>
        ...
        $(document).ready(function() {
            var franchise_id = prompt("Enter a frachise ID?", "");
            StartUpdatingHeartbeat(franchise_id);
        });

        function SendHeartbeatUpdate(franchise_id) {
            WriteScreenMessage("Sending Heartbeat Update...");
            $.ajax({
                type: "POST",
                url: '_update_franchise_heartbeat.php',
                dataType: "json",
                data: {
        ...
        function StartUpdatingHeartbeat(franchise_id) {
            setInterval(function(){SendHeartbeatUpdate(franchise_id)},heartbeat_delay*1000);
        }
