<?php
    # Master Create Tables
    function create_all_tables() {
        log_info("create_all_tables called");
        $fail_count = 0;
        log_info('Creating admin heartbeat table');
        if(create_admin_heartbeat_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating franchise heartbeat table');
        if(create_franchise_heartbeat_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating franchise login table');
        if(create_franchise_login_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating franchises table');
        if(create_franchises_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating league table');
        if(create_league_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating divisions table');
        if(create_divisions_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
       
        log_info('Creating roster players table');
        if(create_roster_players_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating players table');
        if(create_players_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating injuries table');
        if(create_player_injuries_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating data updates table');
        if(create_data_updates_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating draft results table');
        if(create_draft_results_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating player scores table');
        if(create_player_scores_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating free agents table');
        if(create_free_agents_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
        
        log_info('Creating watched players table');
        if(create_watched_players_table()) { log_info('SUCCESS'); } 
        else { log_info('**FAILED**'); $fail_count++; }
